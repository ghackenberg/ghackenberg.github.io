...
import type { AioEvaluationResult } from '../types.js';
...
export function evaluateAioExtractability(target: string): AioEvaluationResult {
  ...
  for (const line of lines) {
    ...
    } else if (currentHeading && trimmed.length > 0 && !trimmed.startsWith('#') && !trimmed.startsWith('|') && !trimmed.startsWith('-') && !trimmed.startsWith('*')) {
  ...
  for (const item of headingsWithFirstParagraph) {
    ...
    if (words >= 30 && words <= 65) {
      directAnswerExamples.push(`Under "${item.heading}": "${item.firstPara.slice(0, 100)}..." (${words} words)`);
    }
  }

  const avgWords = headingsWithFirstParagraph.length > 0
  ...
  if (questionHeadings.length > 0) {
    keyFindings.push(`Found ${questionHeadings.length} question-oriented heading(s) matching search intent.`);
  } else {
  ...
  if (directAnswerExamples.length > 0) {
    keyFindings.push(`Identified ${directAnswerExamples.length} ideal direct answer snippet(s) (30-65 words).`);
  } else {
    concreteRecommendations.push(
  ...
  if (tableCount > 0) {
    keyFindings.push(`Contains structured comparison table(s), highly favored by LLM crawlers for entity extraction.`);
  } else {
    concreteRecommendations.push(
  ...
  if (listCount >= 5) {
    ...
    concreteRecommendations.push('Break down dense prose paragraphs into structured 3-5 item bullet lists or ordered steps.');
  }
